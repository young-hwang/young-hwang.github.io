...
JDBC Connection 설정을 위해 의례적으로 사용을 해오던 구문입니다.
문득 Class.forName으로 클래스를 지정해 줬는데 어떻게 DriverManger에서 Connection 객체를 가져오는지 궁금해져서 해당 부분을 좀더 살펴보기로 하였습니다.
...
- 정적 블록은 클래스가 메모리에 로드 될 때 항상 실행되며 몇 가지 지침을 할 수 있습니다. 
...
클래스 로딩 경쟁 후 JVM은 기본 메소드에서 실행을 시작합니다.
그러나 정적 블록은 클래스가 메모리에 로드 될 때 한 번만 실행되는 코드 블록입니다.
런타임에 변수를 초기화하는 데 사용되기 때문에 정적 초기화 블록이라고도 합니다.
따라서 JVM이 클래스를 메모리에로드 할 때 정적 블록을 실행합니다.
...
Static Method를 호출 후 Item객체를 두개 생성하여 Instance Block, Constructor 실행을 확인하여 보았습니다. 
Static Method에 의해 Static Block이 실행 됨을 확인하였고 Instance Block은 작동하이 않았습니다. 
인스텐스 생성 시 Static Block이 이미 실행이 되었으므로 Instance Block만 실행이 됨을 알 수 있습니다. 
만약 Static Method의 호출이 없었다면 Static Block → Instance Block → Constructor 의 순서로 작동합니다.
